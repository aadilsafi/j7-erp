...
@section('seo-breadcrumb')
    {{ Breadcrumbs::view('breadcrumbs::json-ld', 'sites.settings.accounts.fifth-level.index', $site_id) }}
@endsection

@section('page-title', '5th Level Account')
...
@section('content')
    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ route('sites.settings.accounts.fifth-level.store', ['site_id' => $site_id]) }}" id="fifth-level-accounts" method="get">
                {{ $dataTable->table() }}
            </form>
        </div>
    </div>
...
@section('custom-js')
    ...
    <script>
        ...
        function addNew() {
            location.href = '{{ route('sites.settings.accounts.fifth-level.create', ['site_id' => encryptParams($site_id)]) }}';
        }
