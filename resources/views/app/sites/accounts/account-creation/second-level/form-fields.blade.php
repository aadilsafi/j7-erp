<div class="card" style="border: 2px solid #7367F0; border-style: dashed; border-radius: 0;">
    <div class="card-body">
        <div class="row mb-1">
            ...
            <div class="col-lg-3 col-md-3 col-sm-3 position-relative">
                <label class="form-label fs-5" for="first_level">First Level Code <span
                        class="text-danger">*</span></label>
                <select class="select2" name="first_level" id="first_level">
                    @foreach ($firstLevelAccount as $firstLevel)
                        <option value="{{ $firstLevel->code }}">{{ $firstLevel->name }}({{ $firstLevel->code }})
                        </option>
                    @endforeach
                </select>
                @error('first_level')
                    <div class="invalid-feedback">{{ $message }}</div>
                @else
                    <p class="m-0"><small class="text-muted">Select First Level Account Code.</small></p>
                @enderror
            </div>
