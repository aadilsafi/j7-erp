...
@section('content')
    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ route('sites.settings.accounts.second-level.store', ['site_id' => $site_id]) }}" id="second-level-accounts" method="get">
                {{ $dataTable->table() }}
            </form>
        </div>
    </div>
...
@section('custom-js')
    ...
    <script>
        ...
        function addNew() {
            location.href = '{{ route('sites.settings.accounts.second-level.create', ['site_id' => encryptParams($site_id)]) }}';
        }
