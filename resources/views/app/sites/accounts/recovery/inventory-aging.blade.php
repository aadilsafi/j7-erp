                                    ...
                                    <div class="card-body">
                                        <div class="row mb-1 g-1">
                                            ...
                                            <div class="col-xl-4 col-lg-4 col-md-4 col-sm-12 position-relative">
                                                ...
                                                <select class="select2-size-lg form-select col-filter" id="unit_value"
                                                name="unit_value">
                                                <option value="0" selected>Select Unit</option>
                                                @foreach ($salesPlans as $salesPlan)
                                                    <option value="{{$salesPlan->unit->id}}">{{$salesPlan->unit->name}} - {{$salesPlan->unit->floor_unit_number}}</option>
                                                @endforeach
                                                 </select>
                                            </div>
                            ...
                            <table id="example" class="table table-striped dt-complex-header table"
                                ...
                                <tbody>
                                    ...
                                    @php
                                       ...
                                       $amount_due = $salesPlan->installments->where('date',now())->pluck('amount')->sum();
                                    ...
                                    @endphp
                                        @foreach ( $salesPlan->installments as $key=>$installment )
                                                    <tr>
                                                        <th scope="row">{{$i}}</th>
                                                        <td class="text-nowrap" ><span>{{date_format(new DateTime($installment->date), 'h:i:s')}}</span><br> <span class="text-primary fw-bold">{{date_format(new DateTime($installment->date), 'Y-m-d')}}</span></td>
                                                        <td class="text-nowrap" >{{$salesPlan->unit->name}} - {{$salesPlan->unit->floor_unit_number}}</td>
                                                        <td class="text-nowrap" >{{ $salesPlan->stakeholder_data['full_name']??'' }}</td>
                                    ...
                                    @php
                                    $amount_sum_array = [];
                                            $amount_sum =  collect($amount_installment)->pluck('amount')->sum();
                                            $remaining_amount_sum =  collect($amount_installment)->pluck('remaining_amount')->sum();
                                            $paid_amount =  collect($amount_installment)->pluck('paid_amount')->sum();
                                            $amount_due =  collect($amount_installment)->pluck('amount_due')->sum();
