...
@section('custom-js')
    ...
    <script>
        ...
        function printTemplate(template_id) {
            ...
            let url =
                "{{ route('sites.file-managements.file-buy-back.print', ['site_id' => encryptParams($site_id), 'file_buy_back_id' => ':file_id', 'template_id' =>  ':template_id']) }}"
        ...
        function ApproveModal(site_id,customer_id,unit_id,file_buy_back_id) {
            Swal.fire({
                icon: 'warning',
                title: 'Warning',
                ...
                if (result.isConfirmed) {
                    showBlockUI('#fileBuyBack');
                    let url =
                    '{{ route('sites.file-managements.file-buy-back.approve', ['site_id' => ':site_id' , 'customer_id' => ':customer_id' ,'unit_id' => ':unit_id' ,'file_buy_back_id' => ':file_buy_back_id']) }}'.replace(':site_id', site_id).replace(':customer_id', customer_id).replace(':unit_id', unit_id).replace(':file_buy_back_id', file_buy_back_id);
                    location.href = url;
                }
            });
        }
