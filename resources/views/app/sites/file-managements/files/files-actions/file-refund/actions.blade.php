    ...
    @if (Route::current()->getName() == 'sites.file-managements.file-refund.index')
        @can('sites.file-managements.file-refund.approve')
            ...
            @if (isset($checkFileRefund) && $file_refund_status == 0)
                <a onClick="ApproveModal()" id="approveID"
    ...
    function ApproveModal() {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                let url =
                    "{{ route('sites.file-managements.file-refund.approve', ['site_id' => encryptParams($site_id), 'customer_id' => encryptParams($customer_id), 'unit_id' => encryptParams($unit_id), 'file_refund_id' => encryptParams($file_refund_id)]) }}";
                location.href = url;
            }
        });
    }
