...
@section('seo-breadcrumb')
    {{ Breadcrumbs::view('breadcrumbs::json-ld', 'sites.file-managements.rebate-incentive.index', $site_id) }}
@endsection

@section('page-title', __('Rebate Incentive'))
...
@section('breadcrumbs')
    <div class="content-header-left col-md-9 col-12 mb-2">
        <div class="row breadcrumbs-top">
            <div class="col-12">
                <h2 class="content-header-title float-start mb-0">Rebate Incentive</h2>
                <div class="breadcrumb-wrapper">
                    {{ Breadcrumbs::render('sites.file-managements.rebate-incentive.index', $site_id) }}
                </div>
            </div>
        </div>
    </div>
...
@section('custom-js')
    ...
    <script>
        function ApproveModal() {
            let rebate_id = $('#approveID').attr('rebate_id');
            Swal.fire({
                icon: 'warning',
                title: 'Warning',
                ...
                if (result.isConfirmed) {
                    let url =
                        "{{ route('sites.file-managements.rebate-incentive.approve', ['site_id' => encryptParams($site_id), 'rebate_incentive_id' => ':rebate_incentive_id']) }}"
                        .replace(':rebate_incentive_id', rebate_id);
                    location.href = url;
                }
            });
        }
        ...
        function addNew() {
            location.href =
                '{{ route('sites.file-managements.rebate-incentive.create', ['site_id' => encryptParams($site_id)]) }}';
        }
    </script>
@endsection
