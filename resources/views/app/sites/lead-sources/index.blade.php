...
@section('page-title', 'Lead Sources List')
...
@section('content')
    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ route('sites.lead-sources.destroy-selected', ['site_id' => encryptParams($site_id)]) }}" id="lead-sources-table-form" method="get">
                {{ $dataTable->table() }}
            </form>
        </div>
    </div>
...
@section('custom-js')
    ...
    <script>
        ...
        function addNew() {
            location.href = '{{ route('sites.lead-sources.create', ['site_id' => encryptParams($site_id)]) }}';
        }
    </script>
@endsection
